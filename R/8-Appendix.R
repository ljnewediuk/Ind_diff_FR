...
covariates <- readRDS('results/RSF_outputs/id_hab_covariates.rds')
...
###################################################################
### Supplementary Figure A2: Change in R2 with number of points ###
###################################################################
# Plots number of testing data points in each iteration against
# correlation coefficient. Found no relationship.

melt_correlations <- melt(all_correlations[, c(3:8)], measure.vars = c('individual', 'no_FR', 'ranef', 'gfr'))

melt_correlations$variable <- factor(melt_correlations$variable)
levels(melt_correlations$variable) <- c('Individual', 'Base', 'Ran. Eff.', 'GFR')

# tiff('figures/suppA1.tiff', width = 6, height = 6, units = 'in', res = 300)
...
#####################################################################
### Supplementary Figure A3: Change in Beta with number of points ###
#####################################################################
...
levels(covariates$term) <- c('Coniferous', 'Mixed forest', 'Road avoidance')

# tiff('figures/suppA2.tiff', width = 13, height = 6, units = 'in', res = 300)
...
# tiff('figures/suppA4.tiff', width = 13, height = 6, units = 'in', res = 300)
